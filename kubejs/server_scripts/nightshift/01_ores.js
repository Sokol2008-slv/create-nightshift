...
// Руды НЕ маскируем (решение 26.09): ребята любят копать и лутать данжи. Руды видно и можно
// добыть, но сырьё и всё из него заперто по фазе в 02_items.js (подобрать и хранить можно,
// ставить/надевать/использовать — нет). Вернуть «спящие» руды — поставить true.
var NS_HIDE_ORES = false

function hideOre(id, stage, originalId, replacementId) {
    if (!NS_HIDE_ORES) return
