...
Основной способ — **выделенный сервер** (адрес присылает Жора): «Сетевая игра» → «Добавить сервер».
Steam-приглашения (e4steam) для этой сборки не годятся: лимит Steam ~256 КБ/с, на входе
в мир клиента выкидывает по таймауту. Оставлены для лёгких сценариев:
